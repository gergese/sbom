...
def save_all_library_versions(library_path, file, processed_libraries):
    # 이미 처리한 라이브러리인지 확인
...
def main():
    ...
    with open("sbom_library_versions2.txt", "w") as file:
        pids = get_process_ids()
        for pid in pids:
            file.write(f"Process ID: {pid}\n")
            libraries = get_dynamic_libraries_for_pid(pid)
            for lib in libraries:
                save_all_library_versions(lib, file, processed_libraries)  # 라이브러리 버전을 하나의 파일에 저장
