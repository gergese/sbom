...
def get_lib_type(library_path):
    # 시스템 라이브러리 경로와 외부 라이브러리 경로를 구분
    system_paths = ["/usr/lib", "/lib", "/lib64"]  # 시스템 라이브러리 경로들
...
def parse_readelf_output(library_info):
    ...
    for lib_name, lib_data in library_info.items():
        ...
        # .gnu.version_d 섹션에서 Flags: BASE가 없는 Name 추출
        ...
        # JSON 데이터 구조 생성
        lib_type = get_lib_type(lib_name)  # 라이브러리 타입 결정

        parsed_info.append({
            "lib_name": file_name,  # lib_name 추가
            "lib_path": lib_name,   # 원본 경로 저장
            "lib_type": lib_type,   # 라이브러리 타입 추가
            "lib_version": {
...
def main():
    ...
    library_info_by_pid = []  # PID별 라이브러리 정보를 저장할 리스트

    # 실행 중인 모든 프로세스가 사용하는 라이브러리 정보를 메모리에서 처리합니다.
    pids = get_process_ids()
    target_pid = "109003"  # 특정 PID 설정

    # 특정 PID만 처리하도록 필터링
    pids = [pid for pid in pids if pid == target_pid]

    for pid in pids:
        print(f"처리 중인 PID: {pid}")
        try:
            libraries = get_dynamic_libraries_for_pid(pid)
            pid_libraries = {"PID": pid, "libraries": []}  # 현재 PID의 라이브러리 정보를 저장할 딕셔너리
            ...
            for lib in libraries:
                ...
                if library_version_info:
                    ...
                    if parsed_info:
                        pid_libraries["libraries"].extend(parsed_info)
            ...
            # PID별 정보를 리스트에 추가
            library_info_by_pid.append(pid_libraries)

        except Exception as e:
            print(f"PID {pid} 처리 중 오류 발생: {e}")

    # JSON 파일로 저장
    json_output_file = 'sbom_library_versions_apache2.json'  # JSON 파일로 저장할 경로
    with open(json_output_file, 'w') as json_file:
        json.dump(library_info_by_pid, json_file, indent=4)
